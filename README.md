...
Простая реализация сервиса авторизации, работающего на основе JWT токенов. Сервис управляет пользователями, их ролями, правами доступа, занимается выдачей Access/Refresh токенов. Сервис также предоставляет публичный ключ в виде JWKS, чтобы другие сервисы могли верифицировать подписи выданных токенов самостоятельно
...
Проект содержит гибко расширяемую систему авторизации и управления доступом, в том числе автоматическую инициализацию ролей, учетных записей и прав. Ниже описаны основные особенности.
...
🛠️ В будущем добавится возможность управления этими режимами через переменные окружения.

---

### 🔐 2. Роли

Пользователь может обладать несколькими ролями одновременно. Каждая роль связана с набором **прав доступа**, которые определяют поведение системы авторизации.

📌 **Основные операции с ролями:**

| Метод     | URL                            | Описание                                     |
|-----------|---------------------------------|----------------------------------------------|
| `GET`     | `/api/roles`                   | Получение всех ролей (с пагинацией)          |
| `GET`     | `/api/roles/{roleName}`        | Получение роли по имени                      |
| `POST`    | `/api/roles`                   | Создание новой роли                          |
| `PATCH`     | `/api/roles/{roleName}/permissions` | Обновление прав доступа у роли         |
| `DELETE`  | `/api/roles/{roleName}`        | Удаление роли                                |
...
| `GET`     | `/api/users/{uuid}`              | Получение пользователя по UUID               |
| `POST`    | `/api/users`                     | Создание нового пользователя                 |
| `DELETE`  | `/api/users/{uuid}`              | Мягкое удаление пользователя                 |
| `PUT`     | `/api/users/{uuid}/password` | Смена пароля у пользователя              |
...
- Сканер реализован в виде **интерфейса**, который наследуется от **абстрактного класса**.
- Рекомендуется расширять именно абстрактный класс для добавления новой логики или поведения.
- Права доступа аннотируются специальными метками и автоматически регистрируются при старте приложения.
...
В проекте присутствует множество переменных окружения, которые позволяют гибко влиять на некоторое поведение. В таблице ниже описаны все переменные окружения, их назначение и значения по умолчанию (если присутствуют)
...
Рекомендуется запускать проект с использованием **Docker Compose**. Убедитесь, что у вас установлены следующие инструменты:
...
💡 Совет: при необходимости вы можете изменить порты, настройки базы данных и другие параметры в docker-compose.yml или .env файле.

## 🧪 Запуск тестов

Проект использует **Gradle** для сборки и запуска тестов. Убедитесь, что у вас установлен [Gradle](https://gradle.org/install/) или используйте обертку `./gradlew`, поставляемую с проектом.
