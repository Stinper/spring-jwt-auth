...
Проект использует библиотеку **TestContainers** ([официальный сайт](https://testcontainers.com/)) для запуска тестов репозиториев. Перед запуском тестов убедитесь, что у вас запущен **Docker**, иначе тесты, использующие TestContainers не смогут выполниться.
